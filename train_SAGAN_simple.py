...
from utils import make_train_val_loader, get_path_train, save_loss, check_feat_add, check_mel_default, make_test_loader, save_loss_test
from model.model_default import Lip2SP
from loss import MaskedLoss

from model.SAGAN.model import Discriminator, train_SAGAN_one_epoch, train_SAGAN_PF_epoch
# wandbへのログイン
wandb.login()
...
def make_model(cfg, device):
    ...
    checkpoint_path = '/home/usr1/q70261a/lip2sp_pytorch_all/lip2sp_920_re/check_point/default/face/2022:12:20_16-17-47_transformer_face/mspec80_550.ckpt'
...
def train_SAGAN_loop(epochs, G, D, train_loader, opt_G, opt_D, loss_f, save_path, device, training_method, mixing_prob, cfg, test_loader, test_dataset):
    ...
    for epoch in range(epochs): 
            output_loss_all, dec_output_loss_all, loss_G_all, loss_D_all = train_SAGAN_one_epoch(G, D, opt_D, opt_G, train_loader, training_method, mixing_prob, loss_f, device)
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        train_SAGAN_loop(100, model, D, train_loader, optimizer, optimizer_d, loss_f=loss_f, save_path=save_path, device=device, training_method='ss', mixing_prob=mixing_prob, cfg=cfg, test_loader=test_loader, test_dataset=test_dataset)

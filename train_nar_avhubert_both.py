...
from model.model_nar_avhubert import Lip2SP_NAR_AVHUBERT_BOTH
...
def save_checkpoint(
        ...
        scaler,
        train_loss_list,
        train_mse_loss_dec_output_list,
        train_mse_loss_output_list,
        val_loss_list,
        val_mse_loss_dec_output_list,
        val_mse_loss_output_list,
        epoch,
        ckpt_path,
    ):
    torch.save(
        {
            ...
            'train_mse_loss_dec_output_list': train_mse_loss_dec_output_list,
            'train_mse_loss_output_list': train_mse_loss_output_list,
            'val_loss_list': val_loss_list,
            'val_mse_loss_dec_output_list': val_mse_loss_dec_output_list,
            'val_mse_loss_output_list': val_mse_loss_output_list,
            'epoch': epoch,
        }, 
        ckpt_path,
    )
...
def train_one_epoch(
    ...
    epoch_loss = 0
    epoch_mse_loss_dec_output = 0
    epoch_mse_loss_output = 0
    iter_cnt = 0
    ...
    for batch in train_loader:
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            mse_loss_dec_output = loss_f.mse_loss(dec_output, feature_avhubert, lip_len, max_len=dec_output.shape[-1])
            mse_loss_output = loss_f.mse_loss(output, feature, feature_len, max_len=output.shape[-1])
            loss = mse_loss_dec_output + mse_loss_output

            epoch_loss += loss.item()
            epoch_mse_loss_dec_output += mse_loss_dec_output.item()
            epoch_mse_loss_output += mse_loss_output.item()
            wandb.log({"train_loss": loss})
            wandb.log({"train_mse_loss_dec_output": mse_loss_dec_output})
            wandb.log({"train_mse_loss_output": mse_loss_output})
        ...
        if cfg.train.debug:
            if iter_cnt > cfg.train.debug_iter:
                ...
                check_mel_nar(feature_avhubert[0], dec_output[0], cfg, "mel_avhubert_train", current_time, ckpt_time)
                break
    ...
    epoch_loss /= iter_cnt
    epoch_mse_loss_dec_output /= iter_cnt
    epoch_mse_loss_output /= iter_cnt
    return epoch_loss, epoch_mse_loss_dec_output, epoch_mse_loss_output
...
def val_one_epoch(
    ...
    for batch in val_loader:
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            ...
            wandb.log({"val_mse_loss_dec_output": mse_loss_dec_output})
            wandb.log({"val_mse_loss_output": mse_loss_output})

        iter_cnt += 1
        if cfg.train.debug:
            if iter_cnt > cfg.train.debug_iter:
                check_mel_nar(feature[0], output[0], cfg, "mel_validation", current_time, ckpt_time)
                check_mel_nar(feature_avhubert[0], dec_output[0], cfg, "mel_avhubert_validation", current_time, ckpt_time)
                break
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        if len(cfg.train.module_is_fixed) != 0:
            ...
            for module in cfg.train.module_is_fixed:
                if module == 'lip2sp':
                    ...
                    set_requires_grad_by_name(model, lambda name: name.startswith('spk_emb_layer.'), requires_grad=False)
                    set_requires_grad_by_name(model, lambda name: name.startswith('decoder.'), requires_grad=False)
                if module == 'avhubert_postnet':
                    set_requires_grad_by_name(model, lambda name: name.startswith('avhubert_postnet.'), requires_grad=False)
                    set_requires_grad_by_name(model, lambda name: name.startswith('postnet_out_layer.'), requires_grad=False)
        ...
        for epoch in range(cfg.train.max_epoch - last_epoch):
            ...
            epoch_loss, epoch_mse_loss_dec_output, epoch_mse_loss_output = train_one_epoch(
                model=model, 
            ...
            train_mse_loss_dec_output_list.append(epoch_mse_loss_dec_output)
            train_mse_loss_output_list.append(epoch_mse_loss_output)

            epoch_loss, epoch_mse_loss_dec_output, epoch_mse_loss_output = val_one_epoch(
                model=model, 
            ...
            val_mse_loss_dec_output_list.append(epoch_mse_loss_dec_output)
            val_mse_loss_output_list.append(epoch_mse_loss_output)
            ...
            save_loss(train_mse_loss_dec_output_list, val_mse_loss_dec_output_list, save_path, 'mse_loss_dec_output')
            save_loss(train_mse_loss_output_list, val_mse_loss_output_list, save_path, 'mse_loss_output')

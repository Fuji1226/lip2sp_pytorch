...
from utils import set_config, get_path_train, make_train_val_loader, count_params, save_loss, check_movie
from model.face_gen import Generator, FrameDiscriminator, SequenceDiscriminator, SyncDiscriminator
...
def save_checkpoint(
    ...
    optimizer_gen, optimizer_disc, scheduler_gen, scheduler_disc,
...
def train_one_epoch(gen, frame_disc, seq_disc, sync_disc, train_loader, train_dataset, loss_f, optimizer_gen, optimizer_disc, device, cfg, ckpt_time):
    ...
    for batch in train_loader:
        ...
        data_len = torch.clamp(data_len, min=min(data_len), max=lip.shape[-1])

        output = gen(lip[..., 0], feature, data_len)
        ...
        sync_start_index_fake = random.randint(0, torch.min(data_len).item() - 1 - cfg.train.sync_disc_analysis_len)
        ...
        disc_loss_all = frame_disc_loss + seq_disc_loss + sync_disc_loss
        ...
        mask = 1.0 - make_pad_mask(data_len, lip.shape[-1]).to(torch.float32).unsqueeze(1).unsqueeze(1)   # (B, 1, 1, 1, T)
    ...
    return epoch_frame_disc_loss, epoch_seq_disc_loss, epoch_sync_disc_loss, epoch_gen_l1_loss, epoch_gen_frame_disc_loss, \
        epoch_gen_seq_disc_loss, epoch_gen_sync_disc_loss, epoch_gen_loss_all


def val_one_epoch(gen, frame_disc, seq_disc, sync_disc, val_loader, val_dataset, loss_f, device, cfg, ckpt_time):
    epoch_frame_disc_loss = 0
    epoch_seq_disc_loss = 0
    epoch_sync_disc_loss = 0
    epoch_gen_l1_loss = 0
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        optimizer_disc = torch.optim.Adam(
            params=list(frame_disc.parameters()) + list(seq_disc.parameters()) + list(sync_disc.parameters()),
        ...
        for epoch in range(cfg.train.max_epoch - last_epoch):
            ...
            epoch_frame_disc_loss, epoch_seq_disc_loss, epoch_sync_disc_loss, epoch_gen_l1_loss, epoch_gen_frame_disc_loss, \

...
def main(cfg):
    ...
    model_path = Path(f"~/lip2sp_pytorch/check_point/classifier/face_aligned_0_50_gray/2023:04:30_10-50-07/mspec80_50.ckpt").expanduser()   # F01 recorded and synth
    ...
    for key, path_list in path_all_dict.items():
        ...
        for path in tqdm(path_list):
            ...
            lip = torch.from_numpy(npz_key['lip']).to(device)      # (C, H, W, T)
            lip_len = torch.tensor([lip.shape[-1]]).to(device)
            lip = (lip - lip_mean) / lip_std
            lip = lip.to(torch.float32).unsqueeze(0)  # (1, C, H, W, T)

...
def generate_for_tts_dict(cfg, model, test_loader, dataset, device, save_path, epoch):
    model.eval()
...
def generate_for_jvs(cfg, model, test_loader, device, save_path, epoch):
    model.eval()
    ...
    for batch in test_loader:
        ...
        start_time = time.time()
        with torch.no_grad():
            dec_output, output, logit, att_w = model(text, text_len, xvector)
            #tf_output, _, _ = model(lip=lip, prev=feature)
...
def generate_for_jvs_ids(cfg, model, test_loader, device, save_path, epoch):
    model.eval()
    ...
    for batch in test_loader:
        ...
        text_len = batch['text_len'].to(device)
        filename = batch['filename']
        spk_ids = batch['spk_ids'].to(device)
        
        start_time = time.time()
        with torch.no_grad():
            dec_output, output, logit, att_w = model(text, text_len, spk_ids)
            #tf_output, _, _ = model(lip=lip, prev=feature)
...
def generate_for_tts_vq(cfg, model, test_loader, dataset, device, save_path, epoch):
    model.eval()
    ...
    for batch in test_loader:
        ...
        start_time = time.time()
        with torch.no_grad():
            dec_output, output, logit, att_w, _, _ = model(text, text_len)
            #tf_output, _, _ = model(lip=lip, prev=feature)

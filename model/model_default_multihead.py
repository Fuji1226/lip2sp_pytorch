...
try:
    ...
    from model.transformer_remake_multihead import Encoder, Decoder
    ...
    from .transformer_remake_multihead import Encoder, Decoder
...
class Lip2SP(nn.Module):
    ...
    def forward(self, lip, prev=None, data_len=None, gc=None, training_method=None, mixing_prob=None):
        ...
        if prev is not None:
            if training_method == "tf":
                dec_output, att_list = self.decoder_forward(enc_output, prev, data_len)
                ...
                # 混ぜたやつでもう一回計算させる
                dec_output, att_list = self.decoder_forward(enc_output, mixed_prev, data_len)
        # 推論時
        else:
            dec_output, att_list = self.decoder_inference(enc_output)

        # postnet
        out = self.postnet(dec_output) 
        return out, dec_output, enc_output.clone().detach(), att_list
    ...
    def decoder_inference(self, enc_output, mode="inference"):
        ...
        if self.which_decoder == "transformer":
            for t in range(max_decoder_time_steps):
                if t == 0:
                    dec_output, att = self.decoder(enc_output, mode=mode)
                    tmp = dec_output.clone().detach()
                    #tmp = dec_output
                else:
                    dec_output, att = self.decoder(enc_output, tmp, mode=mode)
    ...
    def prev_degradetion(self, enc_output, prev, data_len, mixing_prob):
        dec_output, att = self.decoder_forward(enc_output, prev, data_len)

...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, cfg, stride=1, downsample=None, relu_type="relu"
    ):
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        for i in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, self.cfg, relu_type=self.relu_type)
            )
    ...
    def extract_features(self, x, padding_mask=None, tgt_layer=None):
        if padding_mask is not None:
            mask_int = (
                ...
                .expand(padding_mask.shape[0], padding_mask.shape[1], x.shape[-1])
                .to(dtype=torch.int, device=x.device)
            )
            x -= x * mask_int
...
class AVHuBERT(nn.Module):
    def __init__(
        ...
        super().__init__()
        sub_cfg = deepcopy(cfg.avhubert)
        sub_cfg.encoder_layers = sub_cfg.sub_encoder_layers
        resnet = ResEncoder(
            relu_type=cfg.avhubert.resnet_relu_type, weights=cfg.avhubert.resnet_weights, cfg=cfg.avhubert
        )
        self.feature_extractor_audio = SubModel(
            resnet=None, input_dim=cfg.avhubert.audio_feat_dim, cfg=sub_cfg
        )
        self.feature_extractor_video = SubModel(
            resnet=resnet, input_dim=resnet.backend_out, cfg=sub_cfg
        )
        self.modality_fuse = cfg.avhubert.modality_fuse
        self.encoder_embed_dim = cfg.avhubert.encoder_embed_dim
        if self.modality_fuse == "concat":
            self.embed = cfg.avhubert.encoder_embed_dim * 2
        elif self.modality_fuse == "add":
            self.embed = cfg.avhubert.encoder_embed_dim
        self.post_extract_proj = (
            nn.Linear(self.embed, cfg.avhubert.encoder_embed_dim)
            if self.embed != cfg.avhubert.encoder_embed_dim
            else None
        )
        self.dropout_input = nn.Dropout(cfg.avhubert.dropout_input)

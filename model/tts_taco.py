from model.taco import PreNet, Attention, ZoneOutCell, TacotronDecoder, ConvEncoder, TacotronDecoderWithQuantizer, JVSDecoder
...
class JVSTacotron(nn.Module):
    ...
    def forward(self, text, text_len, xvector, feature_target=None):
        """
        text : (B, len_text)
        text_len : (B,)
        feature_target : (B, 80, T)
        """
        enc_output = self.encoder(text, text_len) #(B, len_text, 512)
        dec_output, logit, att_w  = self.decoder(enc_output, text_len, feature_target, training_method='tf', mode='tts', xvector=xvector)
        output = self.postnet(dec_output)
...
class TTSTacotronVqEMA(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        ...
        self._vq_vae = VectorQuantizerEMA(num_embeddings=4048, embedding_dim=256,

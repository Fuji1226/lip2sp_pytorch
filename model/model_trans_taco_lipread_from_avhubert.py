...
class Lip2SP(nn.Module):
    def __init__(
        ...
        self.av_hubert = nn.Linear(768, d_model)
        

        # postnet
        self.postnet = Postnet(out_channels, post_inner_channels, out_channels, post_n_layers)

    def forward(self, av_hubert, prev=None, data_len=None, gc=None, training_method=None, mixing_prob=None, use_stop_token=False):
        """
        lip : (B, C, H, W, T)
        prev, out, dec_output : (B, C, T)
        """
        # 推論時にdecoderでインスタンスとして保持されていた結
        av_hubert = av_hubert.transpose(1, 2)
        enc_output = self.encoder(av_hubert, data_len)    # (B, T, C) 
        enc_output = self.av_hubert(enc_output)

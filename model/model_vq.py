...
class Lip2SP_VQ(nn.Module):
    ...
    def forward(self, lip=None, feature=None, feat_add=None, feature_ref=None, data_len=None, gc=None):
        ...
        if lip is not None:
            with torch.no_grad():
                output, feat_add_out, phoneme = self.decoder(quantize, spk_emb, feat_add)
        else:
            output, feat_add_out, phoneme = self.decoder(quantize, spk_emb, feat_add)

        return output, feat_add_out, phoneme, spk_emb, quantize, embed_idx, vq_loss, enc_output, idx_pred, spk_class

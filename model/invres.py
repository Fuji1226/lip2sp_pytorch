...
class InvResNet3D(nn.Module):
    def __init__(self, in_channels, out_channels, inner_channels, layers, dropout, norm_type, up_scale, sq_r, c_attn, s_attn, n_add_channels):
        super().__init__()
        self.conv3d = nn.Sequential(
            NormalConv(in_channels, inner_channels, 2, norm_type),
            nn.Dropout(dropout),

            InvResLayer3D(inner_channels, inner_channels + n_add_channels, 2, norm_type, up_scale, sq_r, kernel_size=7, c_attn=c_attn, s_attn=s_attn),
            nn.Dropout(dropout),

            InvResLayer3D(inner_channels + n_add_channels, inner_channels + n_add_channels * 2, 2, norm_type, up_scale, sq_r, kernel_size=5, c_attn=c_attn, s_attn=s_attn),
            nn.Dropout(dropout),
...
class InvResNet3D_DSOut(nn.Module):
    def __init__(self, in_channels, out_channels, inner_channels, layers, dropout, norm_type, up_scale, sq_r, c_attn, s_attn, n_add_channels):
        super().__init__()
        self.conv3d = nn.Sequential(
            NormalConv(in_channels, inner_channels, 2, norm_type),
...
class InvResLayer3D_NonRes(nn.Module):
    def __init__(self, in_channels, out_channels, stride, norm_type, up_scale, sq_r, kernel_size=None, c_attn=True, s_attn=True):
        super().__init__()
        self.hidden_channels = int(in_channels * up_scale)
        self.pointwise_conv1 = nn.Sequential(
...
class InvResNet3D_DSOut_NonRes(nn.Module):
    def __init__(self, in_channels, out_channels, inner_channels, layers, dropout, norm_type, up_scale, sq_r, c_attn, s_attn, n_add_channels):
        super().__init__()
        self.conv3d = nn.Sequential(
            NormalConv(in_channels, inner_channels, 2, norm_type),
            ...
            InvResLayer3D_NonRes(inner_channels, inner_channels + n_add_channels, 2, norm_type, up_scale, sq_r, kernel_size=7, c_attn=c_attn, s_attn=s_attn),
            nn.Dropout(dropout),

            InvResLayer3D_NonRes(inner_channels + n_add_channels, inner_channels + n_add_channels * 2, 2, norm_type, up_scale, sq_r, kernel_size=5, c_attn=c_attn, s_attn=s_attn),
            nn.Dropout(dropout),

...
try:
    ...
    from model.transformer_module import VectorQuantizer
except:
    ...
    from .transformer_module import VectorQuantizer
...
class Lip2SP(nn.Module):
    def __init__(
        ...
        self.vq = VectorQuantizer(d_model)
    ...
    def forward(self, lip, prev=None, data_len=None, gc=None, training_method=None, mixing_prob=None):
        ...
        vq_output, vq_loss = self.vq(enc_output)

        enc_output = vq_output
        # decoder
        # 学習時
        if prev is not None:
        ...
        if prev is not None:
            return out, dec_output, feat_add_out, vq_loss
        else:

...
from .taco import PreNet, VAE_TacotronDecoder
...
class VQVAE_Redu4_AR(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        emb_dim = 80
...
class VQVAE_Redu2_AR_Post(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        emb_dim = 256
...
class VectorQuantizerEMA(nn.Module):
    ...
    def forward(self, inputs, data_len):
        ...
        if self.training:
            ...
            mask = torch.zeros((len(data_len), max_len, self._num_embeddings), dtype=torch.bool)
            mask_flat_input = torch.zeros((len(data_len), max_len, self._embedding_dim), dtype=torch.bool)
            # マスクの対角成分を1にする
            ...
            for i, length in enumerate(data_len):
                mask_flat_input[i, :length.item(), :] = True
                
            mask = mask.view(-1, self._num_embeddings)
            mask_flat_input = mask_flat_input.view(-1, self._embedding_dim)
            
            encodings = encodings[mask].view(-1, self._num_embeddings)
            tmp_flat_input = flat_input[mask_flat_input].view(-1, self._embedding_dim)
            ...
            dw = torch.matmul(encodings.t(), tmp_flat_input)
...
class VectorQuantizerForFineTuneWithMLM(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, commitment_cost, decay=0.8, epsilon=1e-5):
        ...
        self.mlm = MLMTrainer(n_code=80, d_model=80, mask_prob=0.0, replace_prob=0.0, random_token_prob=0.0)
        """_summary_
        text_lenによる長さを考慮した損失計算
    ...
    def forward(self, inputs, data_len, True_label=None):
        # convert inputs from BCHW -> BHWC
        ...
        # Encoding
        encoding_indices = torch.argmin(distances, dim=1).view(input_shape[0], -1) #(B, T)
    
        #mlm
        mlm_output = self.mlm(encoding_indices, data_len, encoding_indices.device, True_label)
        encoding_indices = mlm_output['pred'].view(-1).unsqueeze(1)
        ...
        # convert quantized from BHWC -> BCHW
        return loss, quantized, perplexity, encoding_indices, mlm_loss

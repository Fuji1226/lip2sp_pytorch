...
from model.model_default_vq import Lip2SP
...
def train_one_epoch(model, train_loader, optimizer, loss_f, device, cfg, training_method, mixing_prob, epoch, ckpt_time):
    ...
    for batch in train_loader:
        ...
        if cfg.train.use_gc:
            output, dec_output, feat_add_out, vq_loss = model(lip=lip, prev=feature, data_len=data_len, training_method=training_method, mixing_prob=mixing_prob, gc=speaker)               
        else:
            output, dec_output, feat_add_out, vq_loss = model(lip=lip, prev=feature, data_len=data_len, training_method=training_method, mixing_prob=mixing_prob)               
        B, C, T = output.shape
    ...
    return epoch_output_loss, epoch_dec_output_loss, epoch_loss_feat_add, epoch_vq_loss
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        for epoch in range(cfg.train.max_epoch - last_epoch):
            ...
            # training
            train_epoch_loss_output, train_epoch_loss_dec_output, train_epoch_loss_feat_add, train_epoch_loss_vq = train_one_epoch(
                model=model, 
            ...
            all_loss = train_epoch_loss_output + train_epoch_loss_dec_output + train_epoch_loss_feat_add + train_epoch_loss_vq 
            ...
            train_epoch_loss_vq_list.append(train_epoch_loss_vq)
            train_all_loss_list.append(all_loss)

            # validation
            val_epoch_loss_output, val_epoch_loss_dec_output, val_epoch_loss_feat_add, val_epoch_loss_vq  = calc_val_loss(
                model=model, 
            ...
            all_loss = val_epoch_loss_output + val_epoch_loss_dec_output + val_epoch_loss_feat_add + val_epoch_loss_vq 
            ...
            val_epoch_loss_vq_list.append(val_epoch_loss_vq)
            val_all_loss_list.append(all_loss)
            ...
            save_loss(train_epoch_loss_vq_list, val_epoch_loss_vq_list, save_path, "vq_loss")
            save_loss(train_all_loss_list, val_all_loss_list, save_path, "all_loss")

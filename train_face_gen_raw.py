...
def make_model(cfg, device):
    ...
    multiframe_disc = MultipleFrameDiscriminator(
        in_channels=cfg.model.in_channels,
        dropout=cfg.train.disc_dropout,
        analysis_len=cfg.train.multiframe_disc_analysis_len,
    )
    seq_disc = SequenceDiscriminator(
        in_channels=cfg.model.in_channels,
        feat_channels=cfg.model.n_mel_channels,
        dropout=cfg.train.disc_dropout,
        analysis_len=cfg.train.seq_disc_analysis_len,
    )
...
def train_one_epoch(
    ...
    for batch in train_loader:
        ...
        lip_len = torch.clamp(lip_len, min=min(lip_len), max=lip.shape[-1])
        ...
        # multiframe disc
        multiframe_start_index = random.randint(0, torch.min(lip_len).item() - cfg.train.multiframe_disc_analysis_len - 1)
        fake_multiframe = multiframe_disc(output.detach()[..., multiframe_start_index:multiframe_start_index + cfg.train.multiframe_disc_analysis_len])
        real_multiframe = multiframe_disc(lip[..., multiframe_start_index:multiframe_start_index + cfg.train.multiframe_disc_analysis_len])
        ...
        # seq disc
        if torch.min(lip_len).item() - 1 - cfg.train.seq_disc_analysis_len < 0:
            seq_start_index = 0
        else:
            seq_start_index = random.randint(0, torch.min(lip_len).item() - 1 - cfg.train.seq_disc_analysis_len)
        fake_seq = seq_disc(
        ...
        real_seq = seq_disc(
            lip[..., seq_start_index:seq_start_index + cfg.train.seq_disc_analysis_len], 
            feature[..., int(seq_start_index * r):int((seq_start_index + cfg.train.seq_disc_analysis_len) * r)]
        )
        seq_disc_loss = F.mse_loss(fake_seq, torch.zeros_like(fake_seq)) + F.mse_loss(real_seq, torch.ones_like(real_seq))

        # sync disc
        sync_start_index = random.randint(0, torch.min(lip_len).item() - 1 - cfg.train.sync_disc_analysis_len)
        sync_start_index_fake = random.randint(0, torch.min(lip_len).item() - 1 - cfg.train.sync_disc_analysis_len)
        while sync_start_index == sync_start_index_fake:
            sync_start_index_fake = random.randint(0, torch.min(lip_len).item() - 1 - cfg.train.sync_disc_analysis_len)

        lip_crop = lip[..., sync_start_index:sync_start_index + cfg.train.sync_disc_analysis_len]
        output_crop = output.detach()[..., sync_start_index:sync_start_index + cfg.train.sync_disc_analysis_len]
        feature_crop_real = feature[..., int(sync_start_index * r):int((sync_start_index + cfg.train.sync_disc_analysis_len) * r)]
        feature_crop_fake = feature[..., int(sync_start_index_fake * r):int((sync_start_index_fake + cfg.train.sync_disc_analysis_len) * r)]
        ...
        # multiframe disc
        multiframe_start_index = random.randint(0, torch.min(lip_len).item() - cfg.train.multiframe_disc_analysis_len - 1)
        fake_multiframe = multiframe_disc(output[..., multiframe_start_index:multiframe_start_index + cfg.train.multiframe_disc_analysis_len])
...
def main(cfg):
    ...
    data_dir, bbox_dir, landmark_dir, train_df, val_df, ckpt_path, save_path, ckpt_time = get_path_train_raw(cfg, current_time)
    train_loader, val_loader, train_dataset, val_dataset = make_train_val_loader_raw(data_dir, bbox_dir, landmark_dir, train_df, val_df, cfg)

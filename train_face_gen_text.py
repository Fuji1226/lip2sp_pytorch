...
from utils import set_config, get_path_train_raw, make_train_val_loader_tts_face_raw_ddp, count_params, save_loss, check_movie
from model.face_gen_text import TransformerFaceGenerator
...
def make_model(cfg):
    model = TransformerFaceGenerator(
        n_vocab=cfg.model.n_vocab,
        enc_n_layers=cfg.model.face_gen_text_enc_n_layers,
        enc_n_head=cfg.model.face_gen_text_enc_n_head,
        enc_d_model=cfg.model.face_gen_text_enc_d_model,
        enc_conv_kernel_size=cfg.model.face_gen_text_enc_conv_kernel_size,
        enc_conv_n_layers=cfg.model.face_gen_text_enc_conv_n_layers,
        dec_n_layers=cfg.model.face_gen_text_dec_n_layers,
        dec_n_head=cfg.model.face_gen_text_dec_n_head,
        dec_d_model=cfg.model.face_gen_text_dec_d_model,
        lip_channels=cfg.model.in_channels,
        prenet_hidden_channels=cfg.model.face_gen_text_prenet_hidden_channels,
        prenet_dropout=cfg.model.face_gen_text_prenet_dropout,
        dec_out_dropout=cfg.model.face_gen_text_dec_out_dropout,
        post_hidden_channels=cfg.model.face_gen_text_post_hidden_channels,
        post_n_layers=cfg.model.face_gen_text_post_n_layers,
        post_kernel_size=cfg.model.face_gen_text_post_kernel_size,
        post_dropout=cfg.model.face_gen_text_post_dropout,
...
def train_one_epoch(run, model, train_loader, train_dataset, optimizer, loss_f, scaler, rank, cfg, ckpt_time):
    ...
    for batch in train_loader:
        ...
        with autocast():
            ...
            dec_output_loss = loss_f.mse_loss(dec_output, lip, lip_len, lip.shape[-1])
            output_loss = loss_f.mse_loss(output, lip, lip_len, lip.shape[-1])

            logit_mask = 1.0 - make_pad_mask(lip_len, lip.shape[-1]).to(torch.float32).squeeze(1)
...
def process(rank, n_gpu, cfg, run):
    ...
    train_loader, val_loader, train_dataset, val_dataset, train_sampler, val_sampler = make_train_val_loader_tts_face_raw_ddp(data_dir, bbox_dir, landmark_dir, train_df, val_df, cfg, rank, n_gpu)
...
def main(cfg):
    ...
    with wandb.init(
        **cfg.wandb_conf.setup, 
        config=wandb_cfg, 
        settings=wandb.Settings(start_method='fork')) as run:

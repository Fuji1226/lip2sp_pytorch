...
def train_one_epoch(model, train_loader, optimizer, loss_f, device, cfg, training_method, mixing_prob, epoch, ckpt_time, scheduler):
    ...
    for batch in train_loader:
        ...
        optimizer.zero_grad()              
        output = model(lip=lip, data_len=data_len)
        
        ctc_output = output['ctc_output']
        #output_to_text(ctc_output[0])
        ...
        if cfg.train.gradient_accumulation_steps > 1:
            loss = ctc_loss / cfg.train.gradient_accumulation_steps
        loss.backward()
    ...
    loss_dict['epoch_ctc_loss'] /= iter_cnt
    return loss_dict
...
def main(cfg):
    ...
    # Dataloader作成
    train_loader, val_loader, train_dataset, val_dataset = make_train_val_loader_lipread(cfg, data_root)
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        for epoch in range(cfg.train.max_epoch - last_epoch):
            ...
            train_ctc_loss_list.append(all_loss['epoch_ctc_loss'])
            ...
            val_ctc_loss_list.append(all_loss['epoch_ctc_loss'])

...
from synthesis import generate_for_train_check_taco_dict_avhubert
...
from model.model_trans_taco_lipread_from_avhubert import Lip2SP
...
def calc_val_loss(model, val_loader, loss_f, device, cfg, training_method, mixing_prob, ckpt_time):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            all_output = model(av_hubert, prev=feature, data_len=data_len, training_method=training_method, mixing_prob=mixing_prob, use_stop_token=True)            
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        for epoch in range(cfg.train.max_epoch - last_epoch):
            ...
            generate_for_train_check_taco_dict_avhubert(
                cfg = cfg,
                model = model,

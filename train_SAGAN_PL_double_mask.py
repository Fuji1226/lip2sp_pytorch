...
from model.SAGAN.mask import train_SAGAN_onemask_epoch, train_SAGAN_PFmask_epoch, Discriminator_suvey_test3
# wandbへのログイン
wandb.login()
...
def train_SAGAN_loop(epochs, G, D_simple, D_PL, train_loader, opt_G, opt_D_simple, opt_D_PL, loss_f, save_path, device, training_method, mixing_prob, cfg, test_loader, test_dataset, ckpt_path):
    ...
    for epoch in range(epochs): 
        ...
        #professor learing
        output_loss_all, dec_output_loss_all, loss_G_all, loss_D_all, corret_cnt, wrong_cnt, change_g_PL, change_d_PL = train_SAGAN_PFmask_epoch(G, D_PL, opt_D_PL, opt_G, train_loader, training_method, mixing_prob, loss_f, device, change_g_PL, change_d_PL, epoch)
...
def main(cfg):
    ...
    D_simple = Discriminator_suvey_test3(PL=True)
    D_PL = Discriminator_suvey_test3(PL=True)

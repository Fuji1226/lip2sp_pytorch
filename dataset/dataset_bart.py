...
class DatasetBART(Dataset):
    ...
    def __getitem__(
        ...
        phoneme_target, phoneme_masked, phoneme_len = self.transform(
            df, data_path, self.class_to_id
        )
        return (
            phoneme_target,
            phoneme_masked,
    ...
    def __call__(self, df, data_path, class_to_id):
        ...
        if sum(phoneme_len_list) > (self.cfg.model.bart.phoneme_max_len - 2):
            start_index += random.randint(
                0, sum(phoneme_len_list) - (self.cfg.model.bart.phoneme_max_len - 2)
            )
        ...
        if len(phoneme_target) != len(phoneme_masked):
            raise ValueError(
                f"phoneme_target_len and phoneme_masked_len should be equal. {data_path}"
            )
        if (
            len(phoneme_target) > self.cfg.model.bart.phoneme_max_len
            or len(phoneme_masked) > self.cfg.model.bart.phoneme_max_len
        ):
            raise ValueError(
                f"phoneme_target_len and phoneme_masked_len should be smaller than self.cfg.model.bart.phoneme_max_len. {data_path}"
            )
...
def collate_time_adjust_bart(batch, cfg):
    ...
    for phoneme_target, phoneme_masked in zip(phoneme_target_list, phoneme_masked_list):
        if len(phoneme_target) > cfg.model.bart.phoneme_max_len:
            ...
            phoneme_target = torch.nn.functional.pad(
                phoneme_target, (0, padding_len), value=0
            )
            phoneme_masked = torch.nn.functional.pad(
                phoneme_masked, (0, padding_len), value=0
            )

...
def save_checkpoint(
    ...
    torch.save({
        ...
        'scheduler_clf': scheduler_clf.state_dict(),
        'scaler': scaler.state_dict(),
        'scaler_clf': scaler_clf.state_dict(),
...
def train_one_epoch(
    ...
    for batch in train_loader:
        ...
        lip_len_both = torch.cat([lip_len, lip_len[video_index]], dim=0)
        spk_emb_both = torch.cat([spk_emb, spk_emb[video_index]], dim=0)
        lang_id_both = torch.cat([lang_id, lang_id[video_index]], dim=0)
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            with torch.no_grad():
                ...
                enc_output_both = torch.cat([audio_enc_output, lip_enc_output], dim=0)
                
            clf_pred_both = domain_classifier(enc_output_both, lip_len_both)
            clf_pred_audio = clf_pred_both[:cfg.train.batch_size]
            clf_pred_lip = clf_pred_both[cfg.train.batch_size:]
            ...
            loss_clf = loss_clf / cfg.train.iters_to_accumulate
            
        scaler_clf.scale(loss_clf).backward()
        if (iter_cnt + 1) % cfg.train.iters_to_accumulate == 0 or (iter_cnt + 1) % (all_iter - 1) == 0:
            scaler_clf.step(optimizer_clf)
            scaler_clf.update()
            optimizer_clf.zero_grad()

        # update audio autoencoder and lip encoder
        domain_classifier = requires_grad_change(domain_classifier, False)
        with torch.autocast(device_type='cuda', dtype=torch.float16):
            audio_enc_output = audio_encoder(feature, feature_len)
            lip_enc_output = lip_encoder(lip[video_index], lip_len[video_index])
            enc_output_both = torch.cat([audio_enc_output, lip_enc_output], dim=0)
            feature_pred_both = audio_decoder(enc_output_both, lip_len_both, spk_emb_both, lang_id_both)
            clf_pred_both = domain_classifier(enc_output_both, lip_len_both)
            feature_pred_audio = feature_pred_both[:cfg.train.batch_size]
            feature_pred_lip = feature_pred_both[cfg.train.batch_size:]
            clf_pred_audio = clf_pred_both[:cfg.train.batch_size]
            clf_pred_lip = clf_pred_both[cfg.train.batch_size:]
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        if cfg.train.which_scheduler == 'exp':
            ...
            scheduler_clf = torch.optim.lr_scheduler.ExponentialLR(
                optimizer_clf, gamma=cfg.train.lr_decay_exp,
            )
            ...
            scheduler_clf = CosineLRScheduler(
        ...
        scaler_clf = torch.cuda.amp.GradScaler()
        ...
        if cfg.train.check_point_start:
            ...
            scheduler_clf.load_state_dict(checkpoint["scheduler_clf"])
            scaler.load_state_dict(checkpoint["scaler"])
            scaler_clf.load_state_dict(checkpoint["scaler_clf"])
        ...
        wandb.watch(domain_classifier, **cfg.wandb_conf.watch)
        ...
        for epoch in range(cfg.train.max_epoch - last_epoch):
            ...
            if cfg.train.which_scheduler == 'exp':
                ...
                wandb.log({"learning_rate_clf": scheduler_clf.get_last_lr()[0]})
                scheduler.step()
                scheduler_clf.step()
            elif cfg.train.which_scheduler == 'warmup':
                wandb.log({"learning_rate": scheduler.optimizer.param_groups[0]['lr']})
                wandb.log({"learning_rate_clf": scheduler_clf.optimizer.param_groups[0]['lr']})
                scheduler.step(epoch)
                scheduler_clf.step(epoch)

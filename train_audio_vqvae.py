...
def save_checkpoint(
    ...
    torch.save({
        ...
        'val_vq_loss_list': val_vq_loss_list,
        'epoch': epoch
    }, ckpt_path)
...
def train_one_epoch(audio_encoder, vq, audio_decoder, train_loader, optimizer, loss_f, device, cfg, ckpt_time):
    epoch_loss = 0
    epoch_mse_loss = 0
    epoch_vq_loss = 0
    iter_cnt = 0
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        if cfg.train.check_point_start:
            ...
            train_vq_loss_list = checkpoint["train_vq_loss_list"]
            val_loss_list = checkpoint["val_loss_list"]
            val_mse_loss_list = checkpoint["val_mse_loss_list"]
            val_vq_loss_list = checkpoint["val_vq_loss_list"]

...
from model.lip_from_vae import Lip2Sp_VQVAE_MLM
...
def train_one_epoch(model, train_loader, optimizer, loss_f, device, cfg, training_method, mixing_prob, epoch, ckpt_time, scheduler):
    ...
    for batch in train_loader:
        ...
        if cfg.train.gradient_accumulation_steps > 1:
            ...
            mlm_loss = mlm_loss / cfg.train.gradient_accumulation_steps            
                
        if epoch >= GRAD_OK_EPOCH:
            loss = output_loss + vq_loss
        else:
            loss = output_loss + vq_loss + enc_loss + mlm_loss
        loss.backward()
...
def main(cfg):
    ...
    mlm_path = '/home/naoaki/lip2sp_pytorch_all/lip2sp_920_re/check_point/vq_vae/code80_dim80/mlm/mspec80_1950.ckpt'
    model = load_from_vqvae_mlm(model, vq_path, mlm_path)
    ...
    for epoch in range(cfg.train.max_epoch - last_epoch):
        ...
        val_mlm_loss_list.append(val_mlm_loss['epoch_enc_loss'])
        #scheduler.step()
        ...
        save_loss(train_mlm_loss_list, val_mlm_loss_list, save_path, 'mlm_loss')

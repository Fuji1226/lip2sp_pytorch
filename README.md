...
## datasetディレクトリの作成
ホームディレクトリにて、`mkdir dataset`で`/dataset`ディレクトリを作成しておいてください。
...
* `--rm`：これを書くことで、作業終了時にコンテナを停止した際、コンテナが自動で削除されます。
...
2. `train_nar.py`を実行してください。
3. `/check_point`というディレクトリが作成され、`1.ckpt`のようなファイルが保存されることを確認してください。

### `generate_nar.py`の実行
1. `/check_point`というディレクトリには、学習時のチェックポイント（モデルパラメータなどを保存したもの）保存されています。ディレクトリ内の一番最後のチェックポイントのパスをコピーし、`/conf/test/nar.yaml`の`model_path`をそのパスに変更してください。例えば、`1.ckpt`と`2.ckpt`がある場合は`2.ckpt`を選択してください（学習時の結果で、検証データに対して一番損失が小さかったチェックポイントを、自動的に選択するように作っています）。
2. `generate_nar.py`を実行してください。

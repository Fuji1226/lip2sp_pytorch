...
from model.model_vq import Lip2SP_VQ
...
def train_one_epoch(model, mi_estimater, train_loader, optimizer, optimizer_mi, loss_f, device, cfg, ckpt_time):
    ...
    for batch in train_loader:
        ...
        ### update mi estimater ###
        with torch.no_grad():
            output, feat_add_out, phoneme, spk_emb, quantize, embed_idx, vq_loss, enc_output, idx_pred, spk_class = model(feature=feature, feat_add=feat_add, feature_ref=feature, data_len=data_len)
        ...
        loss = loss + mse_loss + vq_loss + cfg.train.mi_weight * mi_loss + cfg.train.classifier_weight * classifier_loss
        loss.backward()
    ...
    return epoch_loss, epoch_loss_feat_add, epoch_mse_loss, epoch_vq_loss, epoch_mi_loss, epoch_estimater_loss, epoch_classifier_loss
...
def val_one_epoch(model, mi_estimater, val_loader, loss_f, device, cfg, ckpt_time):
    ...
    for batch in val_loader:
        ...
        with torch.no_grad():
            output, feat_add_out, phoneme, spk_emb, quantize, embed_idx, vq_loss, enc_output, idx_pred, spk_class = model(feature=feature, feature_ref=feature, data_len=data_len)
            estimater_loss = - mi_estimater.loglikeli(spk_emb, quantize)
            mi_loss = mi_estimater(spk_emb, quantize)
        B, C, T = output.shape
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        for epoch in range(cfg.train.max_epoch - last_epoch):
            ...
            # train
            epoch_loss, epoch_loss_feat_add, epoch_mse_loss, epoch_vq_loss, epoch_mi_loss, epoch_estimater_loss, epoch_classifier_loss = train_one_epoch(
                model=model,
            ...
            # validation
            epoch_loss, epoch_loss_feat_add, epoch_mse_loss, epoch_vq_loss, epoch_mi_loss, epoch_estimater_loss, epoch_classifier_loss = val_one_epoch(
                model=model,
                mi_estimater=mi_estimater,

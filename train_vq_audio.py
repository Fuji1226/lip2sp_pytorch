...
from train_ae_audio import save_dem
...
def train_one_epoch(vcnet, mi_estimater, feat_add_predicter, train_loader, optimizer, optimizer_mi, loss_f, device, cfg, ckpt_time):
    ...
    for batch in train_loader:
        ...
        estimater_loss = - mi_estimater.loglikeli(spk_emb, quantize)
        ...
        with torch.no_grad():
            mi_loss = mi_estimater(spk_emb, quantize)
            feat_add_out = feat_add_predicter(out_upsample)
        ...
        loss = mse_loss + vq_loss + cfg.train.mi_weight * mi_loss + cfg.train.classifier_weight * classifier_loss + cfg.train.feat_add_weight * loss_feat_add
        loss.backward()
    ...
    return epoch_loss, epoch_loss_feat_add, epoch_mse_loss, epoch_vq_loss, epoch_mi_loss, epoch_estimater_loss, epoch_classifier_loss, epoch_feat_add_predicter_loss
...
def calc_dem(vcnet, loader_F01, loader_M01, cfg, device):
    ...
    for _ in range(len(loader_F01)):
        ...
        with torch.no_grad():
            output_f, feat_add_out_f, phoneme_f, spk_emb_f, quantize_f, embed_idx_f, vq_loss_f, enc_output_f, idx_pred_f, spk_class_f, out_upsample_f = vcnet(feature=feature_f, feature_ref=feature_m)
            output_m, feat_add_out_m, phoneme_m, spk_emb_m, quantize_m, embed_idx_m, vq_loss_m, enc_output_m, idx_pred_m, spk_class_m, out_upsample_m = vcnet(feature=feature_m, feature_ref=feature_f)
    ...
    cos_sim_emb_list = []
    cos_sim_enc_list = []
    for emb_f, emb_m, enc_f, enc_m in zip(spk_emb_f_list, spk_emb_m_list, quantize_f_list, quantize_m_list):
        # spk_emb
    ...
    cos_sim_emb_list = []
    cos_sim_enc_list = []
    for emb, emb_shuffle, enc, enc_shuffle in zip(spk_emb_f_list, spk_emb_f_list_shuffle, quantize_f_list, quantize_f_list_shuffle):
        # spk_emb
    ...
    cos_sim_emb_list = []
    cos_sim_enc_list = []
    for emb, emb_shuffle, enc, enc_shuffle in zip(spk_emb_m_list, spk_emb_m_list_shuffle, quantize_m_list, quantize_m_list_shuffle):
        # spk_emb
        cos_sim_emb = np.dot(emb, emb_shuffle) / (np.linalg.norm(emb) * np.linalg.norm(emb_shuffle))
        cos_sim_emb_list.append(cos_sim_emb)        
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        for epoch in range(cfg.train.max_epoch - last_epoch):
            ...
            # train
            epoch_loss, epoch_loss_feat_add, epoch_mse_loss, epoch_vq_loss, epoch_mi_loss, epoch_estimater_loss, epoch_classifier_loss, epoch_feat_add_predicter_loss = train_one_epoch(
                vcnet=vcnet,
            ...
            # validation
            epoch_loss, epoch_loss_feat_add, epoch_mse_loss, epoch_vq_loss, epoch_mi_loss, epoch_estimater_loss, epoch_classifier_loss, epoch_feat_add_predicter_loss = val_one_epoch(
                vcnet=vcnet,
                mi_estimater=mi_estimater,
                feat_add_predicter=feat_add_predicter,

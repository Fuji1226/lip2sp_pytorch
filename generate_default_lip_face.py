...
from utils import make_test_loader_lip_face, get_path_test, load_pretrained_model, gen_data_separate, gen_data_concat
...
def generate(cfg, model, pwg, test_loader, dataset, device, save_path):
    ...
    for batch in tqdm(test_loader, total=len(test_loader)):
        ...
        face_sep = gen_data_separate(face, int(cfg.model.input_lip_sec * cfg.model.fps), cfg.model.fps)
        lip_len = lip_len.expand(lip_sep.shape[0])
        spk_emb = spk_emb.expand(lip_sep.shape[0], -1)

        with torch.no_grad():
            output, dec_output, mixed_prev, fmaps, classifier_out, f0_pred = model(lip_sep, lip_len, spk_emb, face=face_sep)
...
def main(cfg):
    ...
    cfg.model.use_lip_and_face = True
    cfg.model.use_f0_predicter = False
    model = make_model(cfg, device)
    
    for num_gen_epoch in num_gen_epoch_list:
        # single speaker
        model_path = Path(f"~/lip2sp_pytorch/check_point/default/lip_and_face/2023:05:05_17-16-23/mspec80_{num_gen_epoch}.ckpt").expanduser()     # ss
        ...
        for data_root, save_path in zip(data_root_list, save_path_list):
            test_loader, test_dataset = make_test_loader_lip_face(cfg, data_root, train_data_root)

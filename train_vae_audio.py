...
from train_ae_audio import make_test_loader, save_dem
...
def save_checkpoint(model, mi_estimater, feat_add_predicter, optimizer, optimizer_mi, scheduler, epoch, ckpt_path):
# def save_checkpoint(model, mi_estimater, feat_add_predicter, optimizer, optimizer_mi, epoch, ckpt_path):
	torch.save({
...
def train_one_epoch(model, mi_estimater, feat_add_predicter, train_loader, optimizer, optimizer_mi, loss_f, device, cfg, ckpt_time):
    epoch_loss = 0
    ...
    for batch in train_loader:
        ...
        ### update mi estimater ###
        with torch.no_grad():
            output, _, phoneme, spk_emb, mu, logvar, z, enc_output, spk_class, out_upsample = model(feature=feature, feature_ref=feature, data_len=data_len)
        
        estimater_loss = - mi_estimater.loglikeli(spk_emb, z)
        ...
        with torch.no_grad():
            mi_loss = mi_estimater(spk_emb, z)
            feat_add_out = feat_add_predicter(out_upsample)

        loss_feat_add = loss_f.mse_loss(feat_add_out, torch.zeros_like(feat_add_out), data_len, max_len=feat_add.shape[-1])
        mse_loss = loss_f.mse_loss(output, feature, data_len, max_len=output.shape[-1])
        classifier_loss = F.cross_entropy(spk_class, speaker)
        # kl_loss = - 0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
        kl_loss = - 0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), dim=-1)
        kl_loss = torch.mean(kl_loss)

        loss = mse_loss + cfg.train.kl_weight * kl_loss + cfg.train.mi_weight * mi_loss + cfg.train.classifier_weight * classifier_loss + cfg.train.feat_add_weight * loss_feat_add
        loss.backward()
    ...
    return epoch_loss, epoch_loss_feat_add, epoch_mse_loss, epoch_kl_loss, epoch_mi_loss, epoch_estimater_loss, epoch_classifier_loss, epoch_feat_add_predicter_loss


def val_one_epoch(model, mi_estimater, feat_add_predicter, val_loader, loss_f, device, cfg, ckpt_time):
    epoch_loss = 0
    epoch_loss_feat_add = 0
    epoch_mse_loss = 0
    epoch_kl_loss = 0
...
def calc_dem(model, loader_F01, loader_M01, cfg, device):
    ...
    for _ in range(len(loader_F01)):
        ...
        with torch.no_grad():
            output_f, _, phoneme_f, spk_emb_f, mu_f, logvar_f, z_f, enc_output_f, spk_class_f, out_upsample_f = model(feature=feature_f, feature_ref=feature_m)
            output_m, _, phoneme_m, spk_emb_m, mu_m, logvar_m, z_m, enc_output_m, spk_class_m, out_upsample_m = model(feature=feature_m, feature_ref=feature_f)

        spk_emb_f = spk_emb_f[0].to('cpu').detach().numpy().copy()    # (C,)
        spk_emb_m = spk_emb_m[0].to('cpu').detach().numpy().copy()    # (C,)
        z_f = z_f[0].to('cpu').detach().numpy().copy()    # (T, C)
        z_m = z_m[0].to('cpu').detach().numpy().copy()    # (T, C)
    ...
    z_f_list_shuffle = random.sample(z_f_list, len(z_f_list))
    z_m_list_shuffle = random.sample(z_m_list, len(z_m_list))

    # 異なる話者で同じ発話内容の場合
    cos_sim_emb_list = []
    cos_sim_enc_list = []
    for emb_f, emb_m, enc_f, enc_m in zip(spk_emb_f_list, spk_emb_m_list, z_f_list, z_m_list):
        # spk_emb
    ...
    cos_sim_emb_list = []
    cos_sim_enc_list = []
    for emb, emb_shuffle, enc, enc_shuffle in zip(spk_emb_f_list, spk_emb_f_list_shuffle, z_f_list, z_f_list_shuffle):
        # spk_emb
    ...
    cos_sim_emb_list = []
    cos_sim_enc_list = []
    for emb, emb_shuffle, enc, enc_shuffle in zip(spk_emb_m_list, spk_emb_m_list_shuffle, z_m_list, z_m_list_shuffle):
        # spk_emb
        cos_sim_emb = np.dot(emb, emb_shuffle) / (np.linalg.norm(emb) * np.linalg.norm(emb_shuffle))
        cos_sim_emb_list.append(cos_sim_emb)        
...
def calc_dem_during_train(cfg, model, device):
    print("calc dem")
...
def main(cfg):
    ...
    with wandb.init(**cfg.wandb_conf.setup, config=wandb_cfg, settings=wandb.Settings(start_method='fork')) as run:
        ...
        optimizer = torch.optim.Adam(
            params=list(model.parameters()) + list(feat_add_predicter.parameters()),
            lr=cfg.train.lr, 
        ...
        for epoch in range(cfg.train.max_epoch - last_epoch):
            ...
            # train
            epoch_loss, epoch_loss_feat_add, epoch_mse_loss, epoch_kl_loss, epoch_mi_loss, epoch_estimater_loss, epoch_classifier_loss, epoch_feat_add_predicter_loss = train_one_epoch(
                model=model,
            ...
            # validation
            epoch_loss, epoch_loss_feat_add, epoch_mse_loss, epoch_kl_loss, epoch_mi_loss, epoch_estimater_loss, epoch_classifier_loss, epoch_feat_add_predicter_loss = val_one_epoch(
                model=model,
                mi_estimater=mi_estimater,
                feat_add_predicter=feat_add_predicter,
            ...
            # dem
            dem_emb_both, dem_enc_both, dem_emb_F01, dem_enc_F01, dem_emb_M01, dem_enc_M01 = calc_dem_during_train(cfg, model, device)

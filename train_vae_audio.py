...
def train_one_epoch(model, mi_estimater, train_loader, optimizer, optimizer_mi, loss_f, device, cfg, ckpt_time):
    ...
    for batch in train_loader:
        ...
        feature, feat_add, data_len = feature.to(device), feat_add.to(device), data_len.to(device)

        ### update mi estimater ###
        with torch.no_grad():
            output, feat_add_out, phoneme, spk_emb, mu, logvar, z, enc_output = model.forward_(feature=feature, feat_add=feat_add, data_len=data_len)
...
def val_one_epoch(model, mi_estimater, val_loader, loss_f, device, cfg, ckpt_time):
    ...
    for batch in val_loader:
        ...
        with torch.no_grad():
            output, feat_add_out, phoneme, spk_emb, mu, logvar, z, enc_output = model.forward_(feature=feature, data_len=data_len)
            estimater_loss = - mi_estimater.loglikeli(spk_emb, z)
            mi_loss = mi_estimater(spk_emb, z)
        B, C, T = output.shape

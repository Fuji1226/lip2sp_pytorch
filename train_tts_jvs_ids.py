...
from model.tts_taco import JVSTacotronSpkid
from loss import MaskedLossTTS

from utils import get_path_jvs_train, make_train_val_loader_jvs, make_pad_mask_tts, check_mel_default, check_attention_weight, save_loss, make_test_loader_jvs
from synthesis_tts import generate_for_jvs_ids
...
def train_one_epoch(model, train_loader, optimizer, loss_f, device, cfg):
    ...
    for batch in train_loader:
        ...
        text = batch["text"]
        stop_token = batch["stop_token"]
        feature_len = batch["feature_len"]
        ...
        dec_output, output, logit, att_w = model(text, text_len, spk_ids, feature_target=feature)
...
def main(cfg):
    ...
    # path
    data_root, save_path, ckpt_path = get_path_jvs_train(cfg, current_time)
    print("\n--- data directory check ---")
    print(f"data_root = {data_root}")
    print(f"ckpt_path = {ckpt_path}")
    print(f"save_path = {save_path}")
    
    train_loader, val_loader, train_dataset, val_dataset = make_train_val_loader_jvs(cfg, data_root)

    test_root = Path(cfg.test.jvs_path).expanduser()
    test_loader, test_dataset = make_test_loader_jvs(cfg, test_root, data_root)

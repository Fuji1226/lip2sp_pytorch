...
def main():
    ...
    video_dir = Path('~/tcd_timit_fps25').expanduser()
    video_path_list = list(video_dir.glob('**/*.mp4'))
    
    for video_path in tqdm(video_path_list):
        landmark_path = Path(str(video_path).replace('tcd_timit_fps25', 'tcd_timit_fps25_landmark').replace('.mp4', '.csv'))
        if not video_path.exists() or not landmark_path.exists():
            continue

        save_path = Path(str(video_path).replace('tcd_timit_fps25', 'tcd_timit_fps25_avhubert_preprocess'))
        if save_path.exists():
            continue
        ...
        if not preprocessed_landmarks:
            ...
            write_video_ffmpeg(frames, str(save_path), str(ffmpeg_path))
        ...
        write_video_ffmpeg(sequence, str(save_path), str(ffmpeg_path))
